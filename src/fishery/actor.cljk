(ns fishery.actor
  "FisheryOperationsActor — the ISCO-08 6222 community fishery actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one fishery operation request
  ...
  The unconditional invariant: the FisheryOperationsAdvisor can never
  directly commit a record the FisheryOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [fishery.advisor :as advisor]
            [fishery.governor :as governor]
            [fishery.store :as store]))

(defn build-graph
  "Build a compiled FisheryOperationsActor graph. `store` implements
  `fishery.store/Store`. `advisor` implements
  `fishery.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :fishery-id (:fishery-id proposal)
                                    :payload proposal}]
